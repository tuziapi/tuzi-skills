...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".tuzi-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".tuzi-skills", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .tuzi-skills/.env file."
    );
  }
