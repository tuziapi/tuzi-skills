...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.tuzi-skills', '.env');
  const userEnv = path.join(os.homedir(), '.tuzi-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== tuzi-post-to-wechat: Permission & Environment Check ===\n');
